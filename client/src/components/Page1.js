...
class Page1 extends Component {
  constructor(props) {
    super(props);
      this.state = { x: 0, y: 0 , crosshair: 0, mask: 0, xcl: 0, ycl: 0, ximp: 0, yimp: 0, value: 1};
  }
  state = {
      value: 1,
      ximp: 0,
...
updateInputValueX(e) {
  ...
  submit = (e) => {
      ...
      const {ximp, yimp, crosshair, mask} = this.state;
      const coords = {ximp, yimp, crosshair, mask};
    axios
  ...
  toggle_crosshair = (e) => {
      if(e.target.checked == true){
	  this.setState({crosshair: 1});
      }else{
	  this.setState({crosshair: 0});
      }
  }
  moveLeft = (e) => {
  ...
  render() {
	...
	<div>
	    <Checkbox onChange = {this.toggle_mask}> Toggle Mask </Checkbox>
	    <Checkbox onChange = {this.toggle_crosshair}> Toggle X-Hair </Checkbox>
